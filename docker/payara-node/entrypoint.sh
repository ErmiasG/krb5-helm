...
function createNewInstance {
    ...
    PAYARA_INSTANCE_NAME=$(echo "${PAYARA_INSTANCE_NAME}" | tail -1)
    # Add created on 
    ASADMIN_COMMAND="${ASADMIN} -I false -T -a -H ${PAYARA_DAS_HOST} -p ${PAYARA_DAS_PORT} -W ${PAYARA_PASSWORD_FILE} set servers.server.${PAYARA_INSTANCE_NAME}.property.creationDate=$(date +%s)"
    echo "${ASADMIN_COMMAND}"
    ${ASADMIN_COMMAND}
...
ls -la /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/

# echo "cp /etc/krb5.conf /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/"
# cp /etc/krb5.conf /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/
# echo "cp /etc/security/keytabs/service.keytab /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/"
# cp /etc/security/keytabs/service.keytab /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/

# ls -la /opt/payara/appserver/glassfish/nodes/${PAYARA_NODE_NAME}/${PAYARA_INSTANCE_NAME}/config/

### Start ###
echo "Starting instance ${PAYARA_INSTANCE_NAME}"
ASADMIN_COMMAND="${ASADMIN} --passwordfile ${PAYARA_PASSWORD_FILE} start-local-instance --node ${PAYARA_NODE_NAME} --sync full --verbose ${PAYARA_INSTANCE_NAME}"
${ASADMIN_COMMAND}
